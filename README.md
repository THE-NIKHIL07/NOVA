...
Nova AI is a modern, high-performance real-time search engine and AI assistant built with Flutter, FastAPI, Groq LLM, and Tavily Web Search. It streams AI responses token-by-token alongside interactive web search citations.
...
- **Real-Time Communication**: `web_socket_client` for persistent streaming
- **UI Components**: `flutter_markdown` for response rendering, `skeletonizer` for loading states, `url_launcher` for external source links
...
- **AI Intelligence**: `groq` (AsyncGroq client running `llama-3.3-70b-versatile` with `llama-3.1-8b-instant` fallback)
...
│   ├── models/               # Data Models (Search Result, Chat Message)
│   ├── pages/                # App Screens
│   │   ├── home_page.dart    # Home Screen with Search Input & Branding
│   │   └── chat_page.dart    # Chat & Answer Streaming Screen
│   ├── services/             # Web & Mobile Service Layer
│   │   └── chat_web_service.dart # WebSocket Client & Reconnection Handler
...
└── server/                   # FastAPI Backend Application
    ...
    │   ├── llm_service.py    # AsyncGroq LLM Streaming Logic
